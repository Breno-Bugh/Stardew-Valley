...
function filterContent(filterType, save = true) {
    ...
    if (npcSection) {
        npcSection.style.display = filterType === 'npc' ? 'block' : 'none';
    }
    ...
    blocks.forEach(block => {
        ...
        if (filterType === 'all') {
            ...
            block.classList.contains('birthdays-block')
        ) {
            block.style.display = 'block';

        } else if (
            filterType === 'events' &&
            block.classList.contains('events-block')
        ) {
            block.style.display = 'block';

        } else if (
            filterType === 'fish' &&
            block.classList.contains('fish-block')
        ) {
            block.style.display = 'block';
